...
static void ac_shrink_tables(CBMAutomaton *ac, int num_states, int max_states) {
    ...
    int alpha_size = ac->alpha_size;
    void *tmp;
    tmp = (__typeof__(tmp))realloc(ac->go_table, (size_t)num_states * alpha_size * sizeof(int));
    if (tmp) {
        ac->go_table = (int *)tmp;
    }
    tmp = (__typeof__(tmp))realloc(ac->output, (size_t)num_states * sizeof(uint64_t));
    if (tmp) {
        ac->output = (uint64_t *)tmp;
    }
    tmp = (__typeof__(tmp))realloc(ac->output_list, (size_t)num_states * sizeof(int));
    if (tmp) {
        ac->output_list = (int *)tmp;
    }
    tmp = (__typeof__(tmp))realloc(ac->output_next, (size_t)num_states * sizeof(int));
    if (tmp) {
