...
static cbm_yaml_node_t *node_new(yaml_type_t type) {
    cbm_yaml_node_t *n = (cbm_yaml_node_t *)calloc(CBM_ALLOC_ONE, sizeof(*n));
    if (n) {
        n->type = type;
    }
    return n;
...
static char *trim_dup(const char *start, const char *end) {
    ...
    char *s = (char *)malloc(len + SKIP_ONE);
    if (!s) {
        return NULL;
    }
    memcpy(s, start, len);
...
static bool peek_is_list(const char *eol, const char *end) {
    ...
    while (peek < end) {
        const char *peek_eol = (const char *)memchr(peek, '\n', (size_t)(end - peek));
        if (!peek_eol) {
            peek_eol = end;
        }
...
static void parse_key_line(const char *content, int content_len, int indent,
    ...
    const char *colon = (const char *)memchr(content, ':', (size_t)content_len);
    if (!colon) {
        return;
    }
