...
static octree_node_t *octree_new(float ox, float oy, float oz, float half) {
    octree_node_t *n = (octree_node_t *)calloc(CBM_ALLOC_ONE, sizeof(*n));
    if (!n)
        return NULL;
    n->ox = ox;
    n->oy = oy;
...
static void compute_call_depth(int n, const int *es, const int *ed, int ne, const char **labels,
    ...
    if (tail == 0) {
        int *in_d = (int *)calloc((size_t)n, sizeof(int));
        if (in_d) {
            for (int e = 0; e < ne; e++) {
                int t = ed[e];
                if (t >= 0 && t < n)
...
cbm_layout_result_t *cbm_layout_compute(cbm_store_t *store, const char *project,
    ...
    if (cbm_store_search(store, &params, &search_out) != CBM_STORE_OK)
        return (cbm_layout_result_t*)calloc(CBM_ALLOC_ONE, sizeof(cbm_layout_result_t));

    int n = search_out.count, total_count = search_out.total;
    if (n == 0) {
        cbm_store_search_free(&search_out);
        cbm_layout_result_t *r = (cbm_layout_result_t *)calloc(CBM_ALLOC_ONE, sizeof(*r));
        if (r)
    ...
    node_id_entry_t *id_map = (node_id_entry_t *)malloc((size_t)n * sizeof(node_id_entry_t));
    if (!id_map) {
        cbm_store_search_free(&search_out);
        cbm_layout_result_t *r = (cbm_layout_result_t *)calloc(CBM_ALLOC_ONE, sizeof(*r));
        if (r) {
    ...
    cbm_edge_t *all_edges = (cbm_edge_t *)malloc((size_t)edge_cap * sizeof(cbm_edge_t));
    int *es = (int *)malloc((size_t)edge_cap * sizeof(int));
    int *ed = (int *)malloc((size_t)edge_cap * sizeof(int));
                ...
                for (int e = 0; e < tc; e++) {
                    ...
                    if (si >= 0 && di >= 0) {
                        if (mapped >= edge_cap) {
                            ...
                            cbm_edge_t *te2 = (cbm_edge_t *)realloc(all_edges, (size_t)nc * sizeof(cbm_edge_t));
                            int *ts = (int *)realloc(es, (size_t)nc * sizeof(int));
                            int *td = (int *)realloc(ed, (size_t)nc * sizeof(int));
                            if (!te2 || !ts || !td) {
                                if (te2)
                                    all_edges = te2;
                                if (ts)
    ...
    int *cdepth = (int *)calloc((size_t)n, sizeof(int));
    const char **lbls = (const char **)malloc((size_t)n * sizeof(char *));
    if (lbls) {
        for (int i = 0; i < n; i++)
    ...
    body_t *bodies = (body_t *)calloc((size_t)n, sizeof(body_t));
    cbm_layout_result_t *result = (cbm_layout_result_t *)calloc(CBM_ALLOC_ONE, sizeof(*result));
    if (!result || !bodies) {
        free(bodies);
        free(deg);
        free(es);
    ...
    result->nodes = (__typeof__(result->nodes))calloc((size_t)n, sizeof(cbm_layout_node_t));
    result->node_count = n;
    ...
    /* 8. Output edges */
    if (mapped > 0) {
        result->edges = (__typeof__(result->edges))calloc((size_t)mapped, sizeof(cbm_layout_edge_t));
